...
# Clone mofneko/.dotfiles
if [ ! -e $DOTFILES_DIR ]; then
  if "$TRAVIS" = "true"; then
    git clone -b $TRAVIS_BRANCH https://github.com/mofneko/dotfiles.git $DOTFILES_DIR
  else
    git clone https://github.com/mofneko/dotfiles.git $DOTFILES_DIR
  fi
fi
cd $DOTFILES_DIR
